...
RSpec.describe "Users Query" do
  ...
  it "Get a list of all existing users" do
    ...
    user_3 = User.create(username: "Brianne McGlynn Ret.3", email: "user-3@email.com", phone: "(346) 733-0000", role: 'lone_ranger', avatar: "https://robohash.org/ipsamomnisenim.png?size=300x300&set=set1")
    ...
    expect(result['data']['allUsers'].first(3)).to eq([
      {"id"=>"#{user_1_id}", "username"=>"Frankie Buckridge1", "role"=>"user"},
      {"id"=>"#{user_2_id}", "username"=>"Jonathon Boehm2", "role"=>"caretaker"},
      {"id"=>"#{user_3_id}", "username"=>"Brianne McGlynn Ret.3", "role"=>"lone_ranger"}
    ])
  end
end